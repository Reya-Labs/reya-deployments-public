...
#
# Usage: prime-cannon-clone-sources.sh [--verify-only] [--lockfile PATH --omnibus PATH]
set -uo pipefail
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --lockfile|--omnibus)
      ...
      if [ "$1" = --lockfile ]; then LOCKFILE="$2"; else OMNIBUS="$2"; fi
      shift 2 ;;
    *) echo "Unknown option: $1" >&2; exit 2 ;;
  esac
done
# Resolve before Node reads so caller-supplied relative and absolute paths both work.
LOCKFILE="$(node -e 'process.stdout.write(require("node:path").resolve(process.argv[1]))' "$LOCKFILE")"
[ -f "$LOCKFILE" ] && [ -f "$OMNIBUS" ] || { echo "Missing lockfile or Cannon config" >&2; exit 2; }
...
export CANNON_PRIME_LOCKFILE="$LOCKFILE"
...
CHAIN_ID="$(node -e "process.stdout.write(String(require(process.env.CANNON_PRIME_LOCKFILE).chainId))")"
...
# ---------------------------------------------------------------------------
TOML_REFS="$(node - "$OMNIBUS" <<'JS'
const {loadCannonfile} = require('@usecannon/cli-devnet');
loadCannonfile(process.argv[2]).then(({def}) => {
  const raw = def.toJson();
  const settings = Object.assign({}, ...Object.values(raw.var || {}));
  process.stdout.write([...new Set(Object.entries(settings).filter(([key]) => key.endsWith('Package')).map(([,ref]) => ref))].sort().join('\n'));
}).catch(error => { console.error(error.message); process.exit(1); });
JS
)" || exit 1
LOCK_REFS="$(node -p "Object.keys(require(process.env.CANNON_PRIME_LOCKFILE).packages).join('\n')" | sort -u)"
...
while IFS=$'\t' read -r REF DEPLOY_CID MISC_CID <&3; do
  ...
  if [ -n "${MISC_CID}" ] && [ -z "${CANNON_PRIME_SKIP_MISC_CHECK:-}" ]; then
    if ! node "${CANNON}" inspect "${REF}" --chain-id "${CHAIN_ID}" >"${FETCH_LOG}" 2>&1 </dev/null ||
       ! grep -Fq "ipfs://${MISC_CID}" "${FETCH_LOG}"; then
      echo "    FAILED artifact pin: ${REF}"
      FAILED=$((FAILED + 1))
    fi
    CODE="$(curl -s -o /dev/null -w '%{http_code}' --max-time 60 --retry 3 --retry-delay 2 </dev/null \
  ...
  const l = require(process.env.CANNON_PRIME_LOCKFILE);
...
if [ "${FAILED}" -ne 0 ] || [ "${TRUNCATED}" -ne 0 ] || [ -n "${MISSING_MISC}" ]; then
  exit 1
fi
