...
The latest completed devnet release is **1.1.8**, executed and published on
19 September 2026 from deployment commit `987a296` (#541): Arbitrum Sepolia Socket
connectors and static withdraw fees for USDC and wETH on Periphery, the Ethereum
Sepolia fees re-emitted as `FeeUpdatedV2`, and the passive pool fee share enabled
on PassivePerp via the fee model V3 `setGlobalFeeParameters` (seven transactions,
no router change). 1.1.7 (17 September, #540) granted the dedicated devnet
...
CI and both devnet package scripts now restore **`reya-devnet-omnibus:1.1.8@main`**,
complete deploy CID **`QmdGZLiFJKtyuHEV5Etb6Cc71zjWtccDnQMYcvG9whYuxF`**.
...
paragraph at 1.1.5; #540 and #541 realigned all four in their completed-record commits.)
...
unchanged 1.1.8 rehearsal from its completed record must make zero protocol
...
```bash
REF=reya-devnet-omnibus:1.1.8@main
