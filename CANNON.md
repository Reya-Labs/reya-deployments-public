...
The latest completed devnet release is **1.1.5**, executed and published on
12 September 2026 from deployment commit `764572382180720221682529d610ae682979ebe9`.
It upgrades Orders Gateway to router 1.1.3 for resting SL/TP maker fills. The
independent auto-exchange signer permissions from 1.1.4 and Core/spot 1.1.3
remain unchanged. The market/collateral mirror first shipped in 1.1.2; the
paragraph above is its historical origin, not the current upgrade baseline.

A baseline is Cannon's record of operations already executed on this chain.
CI and both devnet package scripts now restore **`reya-devnet-omnibus:1.1.5@main`**,
complete deploy CID **`QmYJezJTjDmNzdeoTmevRKo4bcAgdUXhDqbZqJQwuS3oN6`**.
...
unchanged 1.1.5 rehearsal from its completed record must make zero protocol
...
```bash
REF=reya-devnet-omnibus:1.1.5@main
